...
class cbuffer {
    public:
         ...
         * 
         * Costruttore di default che istanzia un cbuffer vuoto.
        **/
        ...
        **/
        explicit cbuffer(size_t capacity) : _buffer(0), _capacity(0), _head(0), _size(0) {
            _buffer = new T[capacity];
            _capacity = capacity;
        
            #ifndef NDEBUG
            std::cout << "cbuffer::cbuffer(size_t)" << std::endl;
            #endif    
        }
         ...
         * 
         * Costruttore fondamentale, istanzia un cbuffer a partire dal reference di un altro cbuffer
        **/
        cbuffer(const cbuffer& original) : _buffer(0), _capacity(0), _head(0), _size(0) {
            _buffer = new T[original._capacity];
        ...
        class iterator {
            ...
            public:
                ...
                reference operator*() const {
                    return *(cb->_buffer + cb->l2f(offset));
                }

                //Ritorna il puntatore al dato riferito dall'iteratore
                pointer operator->() const {
                    return cb._buffer + l2f(offset);
                }
...
std::ostream& operator<<(std::ostream &os, const cbuffer<T> & cb) {
	for (typename cbuffer<T>::size_t i = 0; i < cb.size(); ++i)
		os << cb[i] << " ";
	return os;
}
#endif
